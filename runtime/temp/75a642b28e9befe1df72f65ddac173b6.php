<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:72:"/var/www/us4yo-miraila-admin/public/../admin/order/view/order/lists.html";i:1529567012;s:65:"/var/www/us4yo-miraila-admin/public/../admin/extra/view/main.html";i:1524451346;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<div class="layui-layout layui-layout-admin">
    ...
    <form class="layui-form" action="<?php echo url('Order/lists'); ?>" method="get" style="margin-top:10px;float:left;margin-bottom: -16px;">
        <div class="layui-form-item" style="margin-left:5px;">
            <div class="layui-inline">
                <select name="order_status" lay-verify="required">
                    ...
                    <option value="1" <?php if(\think\Request::instance()->get('order_status') == '1'): ?>selected<?php endif; ?>>未支付</option>
                    <option value="2" <?php if(\think\Request::instance()->get('order_status') == '2'): ?>selected<?php endif; ?>>已支付</option>
                </select>
            </div>
            <div class="layui-inline">
                <input type="text"  name="order_id" placeholder="订单号" value="<?php echo \think\Request::instance()->get('order_id'); ?>" class="layui-input">
            </div>
            <div class="layui-inline">
        ...
        <tbody>
            <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                <tr>
                    ...
                    <td>
                        <?php if(1 == $vo['pay_type']): ?>
                            <span>支付宝</span>
                        <?php elseif(2 == $vo['pay_type']): ?>
                            <span>微信</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php if(1 == $vo['order_status']): ?>
                            <span>未支付</span>
                        <?php elseif(2 == $vo['order_status']): ?>
                            <span>已支付</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php if($vo['pay_time'] != '0000-00-00 00:00:00'): ?>
                            <?php echo $vo['pay_time']; endif; ?>
                    </td>
                    <td>
                        <?php if($vo['order_time'] != '0000-00-00 00:00:00'): ?>
                            <?php echo $vo['order_time']; endif; ?>
                    </td>
                    <td>
                        <?php 
                            if(showHandle('order/Order/viewmore')){
                         ?>
                            <button class="layui-btn layui-btn-small view" data-id=<?php echo $vo['id']; ?> style='font-size:15px;'>查看更多</button>
                        <?php 
                            }
                         ?>
                    </td>
